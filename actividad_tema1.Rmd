...
```{r PIB real}
pwt <- pwt %>% 
  mutate(tasa_cr = (pib - lag(pib,1)) / lag(pib, 1) * 100)

# Gráfico de PIB real
crecimiento <- pwt %>% 
  filter(pais == "China") %>% 
  ggplot(aes(x = anio, y = tasa_cr)) +
  geom_line(size = 1.5, color = "#c22f45") +
  labs(title = "Tasa de crecimiento del PIB de China", 
       caption = "Fuente: Penn World Table",
       x = "", y = "", col = "") +
  #scale_y_continuous(limits = c(0,20)) +
  theme_economist_white(gray_bg = FALSE)
...
# Guardo el gráfico
ggsave(crecimiento, file="figuras/crecimiento_pib.png")
```
...
ggplot(pwt, aes(x = anio, y = k, col = pais)) +
  ...
  labs(title = "Stock de capital en China y EEUU",
       subtitle = "en mil billones (PPP US$ 2011)", 
       caption = "Fuente: Penn World Table",
       x = "", y = "", col = "") +
  theme_economist_white(gray_bg = FALSE) +
  scale_color_manual(values = c("#c22f45", "#7ad2f6"))
